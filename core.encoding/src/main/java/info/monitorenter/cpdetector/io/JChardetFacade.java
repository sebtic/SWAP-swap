/*
 * IClassFileFilter.java cpdetector Copyright (C) 2004 Achim Westermann, created
 * on 03.06.2004 ***** BEGIN LICENSE BLOCK ***** Version: MPL 1.1/GPL 2.0/LGPL
 * 2.1 The contents of this collection are subject to the Mozilla Public License
 ...
 */
public final class JChardetFacade extends AbstractCodepageDetector implements
    nsICharsetDetectionObserver {
  private static final JChardetFacade instance = new JChardetFacade();
   ...
   */
  @Override
  public synchronized Charset detectCodepage(final InputStream in, final int length)
      throws IOException {
    this.Reset();
    int len;
    int read = 0;
  ...
  private Charset guess() {
    Charset ret = null;
    final String[] possibilities = det.getProbableCharsets();
    /*
    ...
    if (possibilities.length == this.amountOfVerifiers) {
      ...
      final String check = possibilities[0];

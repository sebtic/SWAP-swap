/*
 * UnicodeDetector, <enter purpose here>. Copyright (C) 2005 Achim Westermann,
 * Achim.Westermann@gmx.de ***** BEGIN LICENSE BLOCK ***** Version: MPL 1.1/GPL
 * 2.0/LGPL 2.1 The contents of this collection are subject to the Mozilla
 ...
 * limitations under the License. The Original Code is the cpDetector code in
 * [sub] packages info.monitorenter and cpdetector. The Initial Developer of the
 * Original Code is Achim Westermann <achim.westermann@gmx.de>. Portions created
 * by the Initial Developer are Copyright (c) 2007 the Initial Developer. All
 * Rights Reserved. Contributor(s): Alternatively, the contents of this file may
 * be used under the terms of either the GNU General Public License Version 2 or
 ...
 * file under the terms of any one of the MPL, the GPL or the LGPL. ***** END
 * LICENSE BLOCK ***** * If you modify or optimize the code in a useful way
 * please let me know. Achim.Westermann@gmx.de
 */
...
public class UnicodeDetector extends AbstractCodepageDetector {
  private static UnicodeDetector instance;

  public static UnicodeDetector getInstance() {
    if (UnicodeDetector.instance == null) {
      UnicodeDetector.instance = new UnicodeDetector();
    }
    return UnicodeDetector.instance;
  }
  ...
  public Charset detectCodepage(final InputStream in, final int length) throws IOException {
    final byte[] bom = new byte[4]; // Get the byte-order mark, if there is one
    in.read(bom, 0, 4);
    // Unicode formats => read BOM
    @SuppressWarnings("unused")
    final byte b = (byte) 0xEF;
    ...
    if (bom[0] == (byte) 0 && bom[1] == (byte) 0 && bom[2] == (byte) 0xfe && bom[3] == (byte) 0xff) {

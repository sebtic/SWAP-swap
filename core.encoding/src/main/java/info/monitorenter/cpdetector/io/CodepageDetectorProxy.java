 ...
 * Westermann, created on 03.06.2004. ***** BEGIN LICENSE BLOCK ***** Version:
 * MPL 1.1/GPL 2.0/LGPL 2.1 The contents of this collection are subject to the
...
public final class CodepageDetectorProxy extends AbstractCodepageDetector {
   ...
   * @param length The amount of bytes to take into account. This number shouls
   ...
   *      int)
   * @throws IllegalArgumentException if more bytes had to be read from the
   *           input stream than param length or the given input stream does not
   *           support marking.
   */
  @Override
  public Charset detectCodepage(final InputStream in, final int length) throws IOException,
    ...
    Charset ret = null;
    final int markLimit = length;
    final Iterator<ICodepageDetector> detectorIt = this.detectors.iterator();
  ...
  public String toString() {
    ...
    final Iterator<ICodepageDetector> it = this.detectors.iterator();
    int i = 1;
    while (it.hasNext()) {
      ret.append(i);
      ret.append(") ");

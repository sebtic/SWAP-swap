/*
 * AbstractCharsetdetector.java, provides the default implementation for the
 * high-level codepage detection method of interface ICodepageProcessor.
 * Copyright (C) Achim Westermann, created on 19.07.2004, 21:45:47 ***** BEGIN
 * LICENSE BLOCK ***** Version: MPL 1.1/GPL 2.0/LGPL 2.1 The contents of this
 * collection are subject to the Mozilla Public License Version 1.1 (the
 ...
 * Original Code is the cpDetector code in [sub] packages info.monitorenter and
 * cpdetector. The Initial Developer of the Original Code is Achim Westermann
 * <achim.westermann@gmx.de>. Portions created by the Initial Developer are
 * Copyright (c) 2007 the Initial Developer. All Rights Reserved.
 * Contributor(s): Alternatively, the contents of this file may be used under
 * the terms of either the GNU General Public License Version 2 or later (the
 ...
 * terms of any one of the MPL, the GPL or the LGPL. ***** END LICENSE BLOCK
 * ***** * If you modify or optimize the code in a useful way please let me
 * know. Achim.Westermann@gmx.de
 */
...
public abstract class AbstractCodepageDetector implements ICodepageDetector {
   ...
   */
  @Override
  public int compareTo(final ICodepageDetector o) {
    final String other = o.getClass().getName();
    final String mine = this.getClass().getName();
  ...
  public final Reader open(final URL url) throws IOException {
    Reader ret = null;
    final Charset cs = this.detectCodepage(url);
    if (cs != null) {

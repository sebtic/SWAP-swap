...
import org.projectsforge.swap.core.mime.css.CssPropertyHolder;
...
public class CssDocumentHandler implements DocumentHandler {
  ...
  private static final int CACHED_NAMES_MAX_SIZE = CssPropertyHolder.cachedNamesMaxSize.get();
  ...
  public void endDocument(final InputSource source) throws CSSException {
    // optimize storage
    for (final Media media : css.getMedias().values()) {
      media.optimize();
    }

    // prevent cache to go wild
  ...
  public void importStyle(final String uri, final SACMediaList media,
    ...
    for (int i = 0; i < media.getLength(); ++i) {
      importMedias.add(getCachedName(media.item(i)));
    }
  ...
  public void property(final String name, final LexicalUnit value, final boolean important)
      throws CSSException {
    rules.add(new Rule(getCachedName(name), value, important));
  }
  ...
  public void startMedia(final SACMediaList media) throws CSSException {
    currentMedias.clear();
    for (int i = 0; i < media.getLength(); ++i) {
      currentMedias.add(getCSSStyleSheet().getMedia(getCachedName(media.item(i))));
    }
  }

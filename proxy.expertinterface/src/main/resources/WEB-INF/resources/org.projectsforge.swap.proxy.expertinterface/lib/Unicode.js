Loader.define('lib/Unicode', [] , function() {
  // see http://inimino.org/~inimino/blog/javascript_cset

  // Prevent stupid override by user code
  ...
  var CSET = function () {
      ...
      var U = [0]
          ...
          function fS(s) {
              ...
              return fL(filter.call(res.sort(function (a, b) {
                  return a - b
              }), (function (c, i, a) {
                  return !i || a[i - 1] != a[i]
              })))
      ...
      esc.safe = [32, 33];
      forEach.call(['Lu', 'Ll', 'Lt', 'Lm', 'Lo', 'Nd', 'No', 'Pc', 'Pd', 'Ps', 'Pe', 'Pi', 'Pf', 'Po', 'Sm', 'Sc', 'Sk', 'So'], (function (s) {
      ...
      function surrogateSet(cset) {
          ...
          return [fL(all_hi), (full.length ? [
              [fL(full), [0xDC00, 0xE000]]
          ] : []).concat(ret)]
      }

      function sS2RE(surr) {
          var ret = []
          forEach.call(surr, (function (pair) {
  ...
  var rxIsNumber = null;
  var rxIsDigit = null;
  var rxIsLowercaseLetter = null;
  var rxIsUppercaseLetter = null;

  Unicode.isLetter = function (str) {
    if (rxIsLetter == null) {
      var range = Unicode.fromUnicodeGeneralCategory('Lu');
      range = Unicode.union(range, Unicode.fromUnicodeGeneralCategory('Ll'));
      range = Unicode.union(range, Unicode.fromUnicodeGeneralCategory('Lt'));
      range = Unicode.union(range, Unicode.fromUnicodeGeneralCategory('Lm'));
      range = Unicode.union(range, Unicode.fromUnicodeGeneralCategory('Lo'));
      rxIsLetter = new RegExp("^"+Unicode.toRegex(range)+"+$");
    }
    return rxIsLetter.test(str);
  }

  Unicode.isLowercaseLetter = function (str) {
    if (rxIsLowercaseLetter == null) {
      var range = Unicode.fromUnicodeGeneralCategory('Ll');
      rxIsLowercaseLetter = new RegExp("^"+Unicode.toRegex(range)+"+$");
    }
    return rxIsLowercaseLetter.test(str);
  }

  Unicode.isUppercaseLetter = function (str) {
    if (rxIsUppercaseLetter == null) {
      var range = Unicode.fromUnicodeGeneralCategory('Lu');
      rxIsUppercaseLetter = new RegExp("^"+Unicode.toRegex(range)+"+$");
    }
    return rxIsUppercaseLetter.test(str);
  }

  Unicode.isDigit = function (str) {
    if (rxIsDigit == null) {
      rxIsDigit = new RegExp("^[0-9]+$");
    }
    return rxIsDigit.test(str);
  }

  Unicode.isNumber = function (str) {
    if (rxIsNumber == null) {
      var range = Unicode.fromUnicodeGeneralCategory('Nd');
      rxIsNumber = new RegExp("^"+Unicode.toRegex(range)+"+$");
    }
    return rxIsNumber.test(str);
  }
